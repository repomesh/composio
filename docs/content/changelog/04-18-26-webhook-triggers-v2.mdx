...
- V1, V2, and V3 outbound webhook payload formats are unchanged.
- No server-side migration touches your existing trigger instances or auth configs.

You only need to set up a V2 webhook endpoint in two cases:

1. **You want to use one of the new V2 trigger slugs** (e.g. `SLACK_CHANNEL_MESSAGE_RECEIVED`, `SLACK_DIRECT_MESSAGE_RECEIVED`). A V2 endpoint is required at setup; without one, the upsert returns `400` with a `suggestedFix`.
...
If neither applies to you, no action is required.
