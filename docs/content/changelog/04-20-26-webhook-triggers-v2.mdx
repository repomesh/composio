...
Two new Slack triggers ship on V2 — browse them on the [Slack toolkit page](https://platform.composio.dev/marketplace/slack) under **Triggers**, or use the table below to map them from your existing V1 slugs:
...
The signing secret also lives on the endpoint, so the reverse holds too: two different OAuth apps cannot share one V2 endpoint — Composio would only verify signatures from the app whose secret is on file.
