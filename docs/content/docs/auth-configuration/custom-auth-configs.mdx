...
<Callout type="info">
The `initiate()` example below is **safe for custom auth configs**. The `initiate()` retirement announced in the [2026-04-24 changelog](/docs/changelog/2026/04/24) only affects **Composio-managed** auth configs on OAuth1 / OAuth2 / DCR_OAUTH — not the custom OAuth app you just configured here. Custom auth configs continue to work on `initiate()` indefinitely.
</Callout>
