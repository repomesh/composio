...
- **Your own OAuth app on a trigger whose `requires_webhook_endpoint` flag is true** — `PATCH` the signing secret onto the endpoint as part of [Configuring the webhook endpoint](/docs/setting-up-triggers/creating-triggers#configuring-the-webhook-endpoint):
