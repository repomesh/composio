...
- **Provider → Composio (ingress).** Composio handles this for you. For most toolkits there's nothing to do; for V2 toolkits like Slack you [configure a webhook endpoint](/docs/setting-up-triggers/configuring-webhook-endpoints) once per OAuth app.
