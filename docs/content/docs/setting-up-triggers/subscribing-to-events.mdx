...
- **Provider → Composio (ingress).** A webhook endpoint per OAuth app — Composio configures it for you in most cases. When you bring your own OAuth app on a toolkit like Slack or Notion, you [configure the webhook endpoint](/docs/setting-up-triggers/creating-triggers#configuring-the-webhook-endpoint) once per OAuth app.
