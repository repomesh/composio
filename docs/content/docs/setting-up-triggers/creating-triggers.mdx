...
Composio responds to the provider's verification challenge automatically (Slack `url_verification`, Notion's verification token, and so on) — you don't write any handshake code on your side. Once the provider saves the URL, retry the trigger creation call from [Using the SDK](#using-the-sdk).
...
To inspect a single endpoint (handy for checking `verified_at` after a provider re-save):
