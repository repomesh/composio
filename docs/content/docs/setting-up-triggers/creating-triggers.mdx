...
description: Inspect a trigger type, create the instance, and configure the webhook endpoint when required
...
You can create triggers using the [SDK](#using-the-sdk) or the Composio [dashboard](#using-the-dashboard). If the trigger type's `requires_webhook_endpoint` flag is true and you haven't configured an endpoint yet, the create call returns a `400` — follow [Configuring the webhook endpoint](#configuring-the-webhook-endpoint) and retry.
...
print(trigger_type.requires_webhook_endpoint)
# False → create the trigger normally
# True  → see "Configuring the webhook endpoint" below first
...
// {"properties": {"owner": {...}, "repo": {...}}, "required": ["owner", "repo"]}
console.log(triggerType.requiresWebhookEndpoint);
// false → create the trigger normally
// true  → see "Configuring the webhook endpoint" below first
...
Skip this section unless you already know you need it — the trigger type's `requires_webhook_endpoint` flag was true, or the create call returned a `400` saying a webhook endpoint must be configured.
