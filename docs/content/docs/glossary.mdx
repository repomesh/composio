...
The ingress URL Composio issues per OAuth app for webhook triggers, plus the signing secret used to verify each inbound request. Composio configures the endpoint for you in most cases; for some toolkits (today: Slack, Notion) when you bring your own OAuth app, you configure it yourself. See [Configuring the webhook endpoint](/docs/setting-up-triggers/creating-triggers#configuring-the-webhook-endpoint).
</GlossaryTerm>
