...
For SDK custom tools, set `preload: true` on the custom tool or custom toolkit
instead of listing `LOCAL_*` slugs in `preload.tools`. See
...
`preload: { tools: "all" }` in TypeScript) to preload every Composio-managed
tool allowed by the session filters.
...
tool groups that your agent still needs. For example, this session loads Granola
tools upfront while keeping connection management and workbench support
available:
